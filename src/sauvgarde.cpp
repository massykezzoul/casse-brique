...
void Sauvgarde::write(string name){
    ofstream file(".TMPSAUVGARDE.TMP");
    stringstream ss("");
    Brick* b = NULL;
    if (file) {
        for (int i = 0; i < size ; ++i) {
            file<<terrain[i].getX()<<","<<terrain[i].getY()<<","<<terrain[i].getLargeur()<<","<<terrain[i].getHauteur()<<","<<endl;
        ...
        file.close();
        rename(".TMPSAUVGARDE.TMP",name.c_str());
    }
...
void Sauvgarde::charger(int numero_sauvgarde,Window& w,Player& p,Ball& b,Raquette& r,Tab_brick& t) const{
...
void Sauvgarde::print(const Window* w) const{
    ...
    if (size == 0 ){
        ...
        /* Affichage du msg */
        win.print(0,0,separateur);
        win.print(0,1,"Parties Sauvgardés : ");
        win.print(0,2,separateur);
        /* Affichages des scores */
        for(int i = 0;i<size;++i ){
            ss << i<<"- " <<player[i].get_name() << " : " << player[i].get_score() << " Points" << endl;
